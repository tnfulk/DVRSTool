...
class DVRSCalculationEngineTests(unittest.TestCase):
    ...
    def test_700_plan_c_uses_spacing_driven_solution(self) -> None:
        response = self.engine.evaluate(
            CalculationRequest(
                country=Country.UNITED_STATES,
                mobile_tx_low_mhz=799.0,
                mobile_tx_high_mhz=800.0,
            )
        )
        ...
        self.assertEqual(plan_c.proposed_dvrs_rx_range.low_mhz, 803.0)
        self.assertEqual(plan_c.proposed_dvrs_rx_range.high_mhz, 804.0)
        self.assertEqual(plan_c.proposed_dvrs_tx_range.low_mhz, 773.0)
        self.assertEqual(plan_c.proposed_dvrs_tx_range.high_mhz, 774.0)

    def test_700_plan_c_supports_800_mhz_interoperability_layout(self) -> None:
        response = self.engine.evaluate(
            CalculationRequest(
                country=Country.UNITED_STATES,
        ...
        plan_c = next(plan for plan in response.plan_results if plan.plan_id == "700-C")
        self.assertEqual(plan_c.technical_status, TechnicalStatus.VALID)
        self.assertGreaterEqual(plan_c.proposed_dvrs_rx_range.low_mhz, 806.0)
        self.assertLessEqual(plan_c.proposed_dvrs_rx_range.high_mhz, 824.0)
        self.assertGreaterEqual(plan_c.proposed_dvrs_tx_range.low_mhz, 851.0)
        self.assertLessEqual(plan_c.proposed_dvrs_tx_range.high_mhz, 869.0)
        self.assertTrue(any("interoperability profile" in note for note in plan_c.notes))
    ...
    def test_actual_dvrs_single_frequency_still_must_match_native_plan_family_rules(self) -> None:
        response = self.engine.evaluate(
            CalculationRequest(
                country=Country.UNITED_STATES,
                mobile_tx_low_mhz=799.16875,
                mobile_tx_high_mhz=804.84375,
                actual_dvrs_tx_mhz=853.5,
                actual_dvrs_rx_mhz=808.5,
            )
        )
        ...
        self.assertTrue(
            any("Consult a Motorola Presale Engineer before proceeding" in note for note in response.ordering_summary.notes)
        )
    ...
    def test_wide_mobile_block_limits_proposed_ranges_without_extra_warning(self) -> None:
        ...
        self.assertEqual(response.system_summary.system_tx_range.high_mhz, 855.0)
        self.assertEqual(plan_a1.technical_status, TechnicalStatus.VALID)
        self.assertLessEqual(plan_a1.proposed_dvrs_rx_range.width_mhz, 3.0)
        self.assertLessEqual(plan_a1.proposed_dvrs_tx_range.width_mhz, 3.0)
        self.assertGreater(plan_a1.proposed_dvrs_rx_range.width_mhz, 0.0)
    ...
    def test_800_a1_proposed_ranges_preserve_spacing_driven_solution(self) -> None:
        response = self.engine.evaluate(
            CalculationRequest(
                country=Country.UNITED_STATES,
                mobile_tx_low_mhz=806.0125,
        ...
        self.assertEqual(plan_a1.technical_status, TechnicalStatus.VALID)
        self.assertEqual(plan_a1.proposed_dvrs_rx_range.low_mhz, 816.3375)
        self.assertEqual(plan_a1.proposed_dvrs_rx_range.high_mhz, 819.3375)
        self.assertEqual(plan_a1.proposed_dvrs_tx_range.low_mhz, 861.3375)
        self.assertEqual(plan_a1.proposed_dvrs_tx_range.high_mhz, 864.3375)
    ...
    def test_engine_accepts_five_decimal_place_inputs(self) -> None:
        request = CalculationRequest(
            country=Country.UNITED_STATES,
            mobile_tx_low_mhz=803.12345,
            mobile_tx_high_mhz=804.12345,
        )

        response = self.engine.evaluate(request)

        self.assertEqual(response.system_summary.mobile_tx_range.low_mhz, 803.12345)
        self.assertEqual(response.system_summary.system_tx_range.low_mhz, 773.12345)

    def test_700_only_hint_uses_tx_minus_30_for_system_pairing(self) -> None:
        response = self.engine.evaluate(
            CalculationRequest(
                country=Country.UNITED_STATES,
                mobile_tx_low_mhz=794.0,
                mobile_tx_high_mhz=795.0,
                system_band_hint=SystemBandHint.BAND_700_ONLY,
            )
        )

        self.assertEqual(response.system_summary.detected_band, BandFamily.BAND_700)
        self.assertEqual(response.system_summary.system_tx_range.low_mhz, 764.0)
        self.assertEqual(response.system_summary.system_tx_range.high_mhz, 765.0)
    ...
    def test_uhf380_a1_derives_mobile_rx_as_tx_minus_5(self) -> None:
        request = CalculationRequest(
            country=Country.CANADA,
            mobile_tx_low_mhz=410.0,
            mobile_tx_high_mhz=410.05,
        )

        plan_a1 = next(plan for plan in TECHNICAL_PLANS[BandFamily.UHF_380] if plan.id == "UHF380-A1")
        summary = self.engine._build_plan_system_summary(request, BandFamily.UHF_380, plan_a1)

        self.assertEqual(summary.pairing_source, PairingSource.DETERMINISTIC)
        self.assertEqual(summary.mobile_rx_range.low_mhz, 405.0)
        self.assertEqual(summary.mobile_rx_range.high_mhz, 405.05)

    def test_uhf380_a2_derives_mobile_rx_as_tx_plus_5(self) -> None:
        request = CalculationRequest(
            country=Country.CANADA,
            mobile_tx_low_mhz=410.0,
            mobile_tx_high_mhz=410.05,
        ...
        plan_a2 = next(plan for plan in TECHNICAL_PLANS[BandFamily.UHF_380] if plan.id == "UHF380-A2")
        summary = self.engine._build_plan_system_summary(request, BandFamily.UHF_380, plan_a2)

        self.assertEqual(summary.pairing_source, PairingSource.DETERMINISTIC)
        self.assertEqual(summary.mobile_rx_range.low_mhz, 415.0)
        self.assertEqual(summary.mobile_rx_range.high_mhz, 415.05)
    ...
    def test_800_c_is_only_evaluated_when_800_only_is_selected(self) -> None:
        ...
        self.assertIn("800-C", [plan.plan_id for plan in only_800_response.plan_results])
    ...
    def test_800_only_hint_with_700_mhz_dvrs_pair_does_not_leak_vhf_or_uhf_plans(self) -> None:
        response = self.engine.evaluate(
            CalculationRequest(
                country=Country.UNITED_STATES,
                mobile_tx_low_mhz=806.0,
        ...
        self.assertEqual(returned_plan_ids, ["700-A", "700-B"])
        self.assertEqual(valid_plan_ids, ["700-A"])
